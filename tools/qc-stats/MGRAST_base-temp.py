...
def main(args):
    ...
    if not number_of_seqs == 0:
        ...
        if not opts.output:
            sys.stdout.write('QC-FAILED, 0 sequence passed filtering' + "\n")
        else:
            output_path = os.opts.output.dirname
            failed_file = os.path.join(output_path, 'qc-failed')
            out_hdl = open(failed_file, "w")
            out_hdl.write('QC-FAILED, 0 sequence passed filtering' + "\n")
            out_hdl.close()
