...
def aggregate_clusters(geneclus_file):
    ...
    with open(geneclus_file, "r") as reader:
        for line in reader:
            _, contig, _, entries, *_ = line.split("\t")
            contig_id = contig.replace(" ", "-")

            res.setdefault(contig_id, []).extend(
                (entry[0], entry[1]) for entry in entries.split(";"))
    return res
...
def build_attributes(entry_quals, gc_data):
    """Convert the CDS features to gff attributes field for an CDS entry
    """
    locus_tag = entry_quals["locus_tag"][0]
    attributes = []

    attributes.append(["as_notes",
                       _get_value(entry_quals, "note", _clean_as_notes)])
    attributes.append(
        ["as_gene_functions", _get_value(entry_quals, "gene_functions")])
    ...
    if bio_type != "other":
        # add the gene cluter type from the geneclusters.txt file
        attributes.append(["as_gene_clusters",
                           ",".join(
                               gc[1] for gc in gc_data if gc[0] == locus_tag
                           )])
...
def build_gff(embl_file, gclusters, mag=False):
    ...
    for entry in entries:
        ...
        for entry_feature in entry.features:
            ...
            attributes = build_attributes(quals, gc_data)
